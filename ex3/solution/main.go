...
func main() {
	...
	rowFilter := bigtable.RowFilter(bigtable.ChainFilters(bigtable.FamilyFilter("fam"), bigtable.LatestNFilter(2)))

	rowKey := "row1"
	row, err := tbl.ReadRow(ctx, rowKey, rowFilter)
	if err != nil {
		log.Fatalf("Could not read row with key %s: %v", rowKey, err)
	}

	for _, column := range row["fam"] {
		logrus.Infof("cell value: %s", column.Value)
	}
